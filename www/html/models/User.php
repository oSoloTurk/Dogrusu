<?php 
    ...
    class User {
        ...
        function __construct($arr, $isSession = false) {
            ...
            $this->permission_level = $arr['permission_level'] ?? 0;
        ...
        function toJSON(){
            $arr = [];
            $arr["_id"]= $this->id ?? new MongoDB\BSON\ObjectId(null);
            ...
            $arr["permission_level"] = $this->permission_level ?? 0;
            return $arr;
        }

        function toJSONAsIdentitiy($with_password = True){
            $arr = [];
            if($this->email != null) 
                $arr["email"] = $this->email;
            if($this->password_hash != null && $with_password) 
